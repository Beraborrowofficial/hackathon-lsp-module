...
contract LiquidStabilityPool is ERC4626Upgradeable, UUPSUpgradeable {
    ...
    function mint(
        ...
        uint fee = shares.mulDiv(BP, BP - _entryFeeBP(), Math.Rounding.Ceil) - shares;
    ...
    function _burn(ILiquidStabilityPool.LSPStorage storage $, uint shares, uint _totalSupply, address _owner) private returns (uint nectAmount, uint fee) {
        ...
        if (_totalSupply != 0) {
            nectAmount = (shares - fee).mulDiv($.balanceData.balance[asset()], _totalSupply, Math.Rounding.Floor);
        }
    ...
    function _withdrawCollAndExtraAssets(
        ...
        for (uint i; i < collLength; i++) {
            ...
            amounts[i] = shares.mulDiv(balanceWithUnlockedEmissions, _totalSupply, Math.Rounding.Floor);
        ...
        for (uint i; i < extraAssetsLength; i++) {
            ...
            amounts[idx] = shares.mulDiv(balanceWithUnlockedEmissions, _totalSupply, Math.Rounding.Floor);
            tokens[idx] = token;
    ...
    function previewMint(uint netShares) public view override returns (uint) {
        uint totalShares = netShares.mulDiv(BP, BP - _entryFeeBP(), Math.Rounding.Ceil);
    ...
    function previewWithdraw(uint assets) public view override returns (uint) {
        ...
        uint totalShares = netShares.mulDiv(BP, BP - _exitFeeBP(), Math.Rounding.Ceil);
        return totalShares;
    }
